...
export default function RunDetailPage() {
  ...
  const { data: run } = useQuery({
    queryKey: ["eval-run-info", appId, runId],
    queryFn: () => resultsApi.getRun(appId!, runId!),
    enabled: !!appId && !!runId,
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Run config info strip ───────────────────────────────── */}
      {run && (
        <div className="flex flex-wrap gap-x-6 gap-y-1.5 text-xs text-gray-500 bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5">
          <span><span className="font-medium text-gray-700">RAG Version</span>&nbsp; {run.rag_version}</span>
          <span><span className="font-medium text-gray-700">Golden Set</span>&nbsp; {run.golden_set_version}</span>
          <span><span className="font-medium text-gray-700">Answer Mode</span>&nbsp; {run.answer_mode || "—"}</span>
          <span><span className="font-medium text-gray-700">Identity</span>&nbsp; {run.identity || "—"}</span>
          <span>
            <span className="font-medium text-gray-700">RACL</span>&nbsp;
            {run.racl_entity_ids.length > 0 ? run.racl_entity_ids.join(", ") : "*"}
          </span>
          <span><span className="font-medium text-gray-700">Date</span>&nbsp; {new Date(run.started_at).toLocaleString()}</span>
        </div>
      )}
              ...
              <div className="space-y-2">
                ...
                {result.retrieved_doc_ids.length > 0 && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {result.retrieved_doc_ids.slice(0, 20).map((id, idx) => {
                        const isExpected = result.reference_doc_ids.includes(id);
                        const chunk = result.chunk_signals?.find((c) => c.doc_id === id);
                        const chunkId = chunk?.chunkId ?? null;
                        const label = chunkId ? `${chunkId} - ${id}` : id;
                        return (
                          <span key={id} className={cn(
                            "inline-flex items-center gap-1 pl-1 pr-2 py-0.5 rounded font-mono text-xs",
                            isExpected ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"
                          )}>
                            <span className={cn(
                              "text-[10px] font-bold w-5 h-4 flex items-center justify-center rounded",
                              isExpected ? "bg-green-200 text-green-800" : "bg-gray-200 text-gray-600"
                            )}>
                              {idx + 1}
                            </span>
                            {isExpected && <span className="text-green-600 font-bold">✓</span>}
                            {label}
                          </span>
                        );
                      })}
