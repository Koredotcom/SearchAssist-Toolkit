...
import {
  BarChart3, ChevronRight, TrendingUp, TrendingDown, Minus, Trash2, Loader2,
} from "lucide-react";
        ...
        {isRunning ? (
          <div className="flex flex-col gap-1 min-w-[80px]">
            <span className="text-violet-700 font-medium">
              {run.done_cases}/{run.total_cases}
            </span>
            <div className="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
              <div
                className="h-full bg-violet-500 rounded-full transition-all duration-500"
                style={{ width: run.total_cases > 0 ? `${(run.done_cases / run.total_cases) * 100}%` : "0%" }}
              />
            </div>
          </div>
        ) : (
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full", colors[status] ?? "bg-gray-100 text-gray-600")}>
