...
export default function EvaluatePage() {
  ...
  // Per-run overrides
  const [raclEntityIdsOverride, setRaclEntityIdsOverride] = useState("");
  ...
  const startMutation = useMutation({
    mutationFn: () =>
      evaluationApi.start(appId!, {
        ...
        identity: enableRacl && userEmail.trim() ? userEmail.trim() : null,
        answer_mode_override: answerModeOverride,
        question_types: selectedQTypes.length > 0 ? selectedQTypes : null,
        racl_entity_ids_override: (() => {
          const ids = raclEntityIdsOverride.split(",").map((s) => s.trim()).filter(Boolean);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl p-6 space-y-5">
              ...
              {([
                {
                  ...
                  icon: Zap,
                  title: "Judge-based Scoring",
                  desc: "LLM judge scores retrieved text for coherence, groundedness & relevance (Agent Platform: uses top chunk text, no separate LLM answer)",
                  color: "blue",
                },
                {
                  ...
                  desc: "Verdict based on doc/chunk rank — no judge required",
                  color: "amber",
                },
          ...
          <div className="pt-2 border-t border-gray-100">
            ...
            {/* Per-run RACL entity IDs override */}
            <div className="pt-3 mt-3 border-t border-gray-100">
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">
                  RACL Entity IDs Override
                </label>
                <input
                  type="text"
                  value={raclEntityIdsOverride}
                  onChange={(e) => setRaclEntityIdsOverride(e.target.value)}
                  placeholder="Leave blank to use app default"
                  className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500"
                />
                <p className="text-xs text-gray-400 mt-1">
                  Comma-separated entity IDs for row-level access control. Overrides the app-level RACL Entity IDs for this run only.
                </p>
              </div>
            </div>
            ...
            <div className="pt-3 mt-3 border-t border-gray-100">
              ...
              {!enableRacl ? (
                <p className="text-xs text-gray-400">
                  Queries run with the app&apos;s default identity. Enable to override the <code>input.identity</code> for every query in this run.
                </p>
              ) : (
                <div>
                  <input
        ...
        <div className="space-y-4">
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            ...
            <div className="space-y-2 text-sm">
              ...
              />
              {raclEntityIdsOverride.trim() && (
                <Row label="RACL IDs (override)" value={raclEntityIdsOverride.trim()} />
              )}
            </div>
          ...
          <ConfirmDialog
            ...
            description="Please verify the settings below before starting."
          ...
          >
            {(() => {
              const effectiveIdentity = enableRacl && userEmail.trim()
                ? userEmail.trim()
                : currentApp?.identity || "—";
              const effectiveRacl = raclEntityIdsOverride.trim()
                ? raclEntityIdsOverride.trim()
                : (currentApp?.racl_entity_ids?.length ? currentApp.racl_entity_ids.join(", ") : "*");
              return (
                <div className="rounded-md border border-gray-200 bg-gray-50 px-4 py-3 space-y-2 text-sm">
                  <div className="flex justify-between gap-4">
                    <span className="text-gray-500 shrink-0">Ground Truth</span>
                    <span className="font-medium text-gray-800 text-right">{selectedVersion}</span>
                  </div>
                  <div className="flex justify-between gap-4">
                    <span className="text-gray-500 shrink-0">RAG Version</span>
                    <span className="font-medium text-gray-800 text-right">{ragVersion}</span>
                  </div>
                  <div className="flex justify-between gap-4">
                    <span className="text-gray-500 shrink-0">Identity</span>
                    <span className="font-medium text-gray-800 text-right font-mono">{effectiveIdentity}</span>
                  </div>
                  <div className="flex justify-between gap-4">
                    <span className="text-gray-500 shrink-0">RACL Entity IDs</span>
                    <span className="font-medium text-gray-800 text-right font-mono">{effectiveRacl}</span>
                  </div>
                  <div className="flex justify-between gap-4">
                    <span className="text-gray-500 shrink-0">Answer Mode</span>
                    <span className="font-medium text-gray-800 text-right">
                      {effectiveAnswerMode === "answer_generation" ? "Answer Generation" : "Extraction"}
                      {answerModeOverride && <span className="text-xs text-gray-400 ml-1">(override)</span>}
                    </span>
                  </div>
                  <div className="flex justify-between gap-4">
                    <span className="text-gray-500 shrink-0">Cases</span>
                    <span className="font-medium text-gray-800 text-right">
                      {effectiveCases} of {totalAvailable}
                      {limitCases && sampleMode === "random" ? " (random)" : ""}
                    </span>
                  </div>
                </div>
              );
