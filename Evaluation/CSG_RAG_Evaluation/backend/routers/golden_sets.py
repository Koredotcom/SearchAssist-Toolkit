...
#   1. doc_id / docId / reference_doc_ids → direct ID, matched against chunk["doc_id"]
...
_STANDARD_COLS = {
    ...
    # Post-alias metadata-only keys — excluded from custom-field chunk matching
    "_source_name", "_subtype",
    # Wolken GT export — chunk-level identifier; ignored for doc-level eval
    "chunkid",
}

# GT CSV export column aliases → upload-standard names.
# Applied (before any lookup) to both _parse_csv and _parse_excel.
_GT_COLUMN_ALIASES: dict[str, str] = {
    "query":                 "question",
    "expected record title": "record_title",
    "query_category":        "question_type",
    "expected source":       "_source_name",   # GT source label — metadata only; see note below
    "source name":           "_source_name",   # duplicate path, same target
    "subtype":               "_subtype",        # metadata-only
}

# GT "Expected Source" values → Kore.ai sys_content_type values used in chunk filters.
# The GT CSV stores human-readable source labels; Kore.ai expects its internal type strings.
# This mapping is used at eval time (per-row filter mode) so RAG queries are scoped
# to the correct source. Values not in this map are passed through unchanged.
_GT_SOURCE_TO_KOREAI: dict[str, str] = {
    "jira":               "jira",
    "confluencecloud":    "confluence",
    "confluence":         "confluence",
    "web":                "web",
    "wolkenservicedesk":  "wolkenServiceDesk",
    "wolken":             "wolkenServiceDesk",
}
...
def _resolve_match_spec(app_id: str, row_norm: dict[str, str]) -> tuple[list[dict], list[str]]:
    ...
    for col in _DOC_ID_COLS:
        ...
        if val:
            ids = _parse_ids(val)
            return [{"field": "doc_id", "value": i} for i in ids], ids
    ...
    for col in _TITLE_COLS:
        ...
        if title:
            ...
            if doc_id:
                spec.insert(0, {"field": "doc_id", "value": doc_id})
            return spec, ids
...
def _apply_gt_aliases(norm: dict[str, str]) -> dict[str, str]:
    """Remap GT-export column names to upload-standard names.

    Operates on a dict already lowercased+stripped.
    - GT aliases are applied only when the target key is not already present
      (prevents clobbering a real 'question' column if a file has both 'query'
      and 'question').
    - The GT 'Expected Source' label is stored as '_source_name' (metadata only).
      A normalised sys_content_type is derived separately via _GT_SOURCE_TO_KOREAI
      and stored under 'sys_content_type' so per-row source filtering works.
    """
    out: dict[str, str] = {}
    for k, v in norm.items():
        target = _GT_COLUMN_ALIASES.get(k, k)
        # Never overwrite a value already set by a proper standard-named column
        if target not in out or k not in _GT_COLUMN_ALIASES:
            out[target] = v

    # Derive sys_content_type from the GT source label when not already present
    if "sys_content_type" not in out and "_source_name" in out:
        koreai_type = _GT_SOURCE_TO_KOREAI.get(out["_source_name"].lower().replace(" ", ""))
        if koreai_type:
            out["sys_content_type"] = koreai_type
...
def _parse_csv(content: bytes, app_id: str) -> list[dict]:
    ...
    rows: list[dict] = []
    for row in reader:
        norm = _apply_gt_aliases({k.strip().lower(): (v or "").strip() for k, v in row.items()})
        q = norm.get("question", "")
        if not q:
            continue
...
def _parse_excel(content: bytes, app_id: str) -> list[dict]:
    ...
    for row_vals in rows_iter:
        norm = _apply_gt_aliases({h: _cell(row_vals, h) for h in headers if h})
        q = norm.get("question", "")
        if not q:
            continue
        match_spec, ref_ids = _resolve_match_spec(app_id, norm)
...
def get_test_cases(app_id: str, version: str):
    ...
    for r in rows:
        ...
        result.append({
            ...
            "reference_match_spec": r.get("reference_match_spec", []),
