...
@router.get("/{run_id}/info", response_model=EvalRunResponse)
def get_run_info(app_id: str, run_id: str):
    """Return the eval_run row for a single run (identity, racl, answer_mode, etc.)."""
    if not get_app(app_id):
        raise HTTPException(404, "App not found")
    ...
    verdicted = run.get("verdicted_cases") or 0
    denominator = verdicted if verdicted > 0 else (run.get("total_cases") or 1)
    return {**run, "pass_rate": round(passed / denominator, 4), "done_cases": 0}
