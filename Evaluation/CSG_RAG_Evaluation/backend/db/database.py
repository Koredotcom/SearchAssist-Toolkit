...
def _migrate(conn: sqlite3.Connection) -> None:
    migrations = [
        ...
        # ── New Kore.ai Agent Platform API credentials ────────────────────────
        "ALTER TABLE app_config ADD COLUMN process_id TEXT NOT NULL DEFAULT ''",
        "ALTER TABLE app_config ADD COLUMN cf_version_id TEXT NOT NULL DEFAULT ''",
        "ALTER TABLE app_config ADD COLUMN x_api_key TEXT NOT NULL DEFAULT ''",
        "ALTER TABLE app_config ADD COLUMN identity TEXT NOT NULL DEFAULT ''",
        "ALTER TABLE app_config ADD COLUMN sources TEXT NOT NULL DEFAULT ''",
        "ALTER TABLE eval_run ADD COLUMN identity TEXT NOT NULL DEFAULT ''",
        "ALTER TABLE eval_run ADD COLUMN racl_entity_ids TEXT NOT NULL DEFAULT '[]'",
        "ALTER TABLE eval_run ADD COLUMN answer_mode TEXT NOT NULL DEFAULT ''",
    ]
        ...
        conn.execute(
            """INSERT INTO app_config
               (app_id, name, host_url, bot_id, stream_id,
                ...
                process_id, cf_version_id, x_api_key, identity, sources)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (app_id, data["name"], data.get("host_url", ""), data.get("bot_id", ""),
             data.get("stream_id"), data.get("client_id", ""),
             data.get("client_secret", ""), data.get("jwt_token", ""),
...
def _decode_app_row(row) -> dict:
    ...
    # Add masked preview; keep full key in dict for internal use (search.py reads it)
    key = d.get("x_api_key", "")
    d["x_api_key_preview"] = f"...{key[-4:]}" if len(key) >= 4 else ("set" if key else "")
    return d
...
def get_api_key(app_id: str, provider: str) -> str | None:
    ...
    Priority: app-specific key in DB → environment variable fallback.
    This lets the env var act as a global default without requiring every
    app to have its key saved explicitly.
    """
    app = get_app(app_id)
    db_key = ((app or {}).get(f"{provider}_key") or "").strip()
    if db_key:
        return db_key
...
def mark_stale_jobs_failed() -> int:
    """On server startup, mark any jobs/runs left in 'running' state as failed.

    These are orphans from a previous server process that crashed or was restarted
    mid-evaluation. Returns the number of rows updated.
    """
    with get_db() as conn:
        n_jobs = conn.execute(
            """UPDATE job SET status='failed', error='Server restarted — job interrupted',
                              updated_at=datetime('now')
               WHERE status='running'""",
        ).rowcount
        n_runs = conn.execute(
            """UPDATE eval_run SET status='failed', finished_at=datetime('now')
               WHERE status='running'""",
        ).rowcount
    return n_jobs + n_runs
        ...
        conn.execute(
            """INSERT INTO eval_run
               (run_id, app_id, rag_version, judge_model, golden_set_version, trigger, total_cases,
                identity, racl_entity_ids, answer_mode)
               VALUES (?,?,?,?,?,?,?,?,?,?)""",
            (run["run_id"], run["app_id"], run.get("rag_version", "unknown"),
             ...
             run.get("identity", ""),
             json.dumps(run.get("racl_entity_ids") or []),
             run.get("answer_mode", "")),
        )
...
def list_eval_runs(app_id: str) -> list[dict]:
    with get_db() as conn:
        rows = conn.execute(
            """SELECT er.*,
                      (SELECT COUNT(*) FROM eval_result WHERE run_id = er.run_id) AS done_cases
               FROM eval_run er
               WHERE er.app_id=?
...
def get_eval_results(run_id: str) -> list[dict]:
    with get_db() as conn:
        ...
        for r in rows:
            ...
            d["chunk_signals"] = json.loads(d.get("chunk_signals") or "[]")
            result.append(d)
        return result
