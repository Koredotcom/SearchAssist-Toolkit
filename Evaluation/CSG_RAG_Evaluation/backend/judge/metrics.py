...
def qualified_chunks_count(chunk_signals: list[dict]) -> int:
    """Number of chunks in the retrieval set.

    Old API: chunkQualified=True. New API: all returned chunks are the retrieval set.
    """
    if not chunk_signals:
        return 0
    qualified = [c for c in chunk_signals if c.get("chunkQualified") is True]
    return len(qualified) if qualified else len(chunk_signals)
