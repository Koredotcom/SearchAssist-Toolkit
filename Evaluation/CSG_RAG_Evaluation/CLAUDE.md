...
- `app_config` stores Kore.ai Agent Platform credentials: `process_id` (a-…), `cf_version_id` (cf-…), `x_api_key` (kg-…), `identity`, `sources`, `racl_entity_ids` per app. Legacy fields `host_url`/`bot_id`/`jwt_token` remain in schema for backward compat with old connectors API.
...
- **RAG query API**: Kore.ai Agent Platform — `POST https://agent-platform.kore.ai/api/v1/process/{process_id}/version/{cf_version_id}` with `x-api-key` header. No LLM-generated answer; returns retrieved chunks only.
