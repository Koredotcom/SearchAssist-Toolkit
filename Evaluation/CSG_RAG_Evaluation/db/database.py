...
def init_db() -> None:
    ...
    try:
        ...
        existing = {row[1] for row in conn.execute("PRAGMA table_info(test_case)")}
        if "reference_title" not in existing:
            conn.execute("ALTER TABLE test_case ADD COLUMN reference_title TEXT")
            conn.commit()
    finally:
        conn.close()
        ...
        conn.execute(
            ...
            INSERT OR REPLACE INTO test_case
                (tc_id, golden_set_version, question, expected_answer, expected_behavior,
                 ...
                 reference_title, generation_metadata, rationale)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                tc["tc_id"],
...
def insert_agent_scores_keep(tc_id: str) -> None:
    """Insert a synthetic KEEP decision so GT test cases pass the active filter."""
    with get_db() as conn:
        conn.execute(
            "INSERT OR IGNORE INTO agent_scores (tc_id, decision) VALUES (?, 'KEEP')",
            (tc_id,),
        )
