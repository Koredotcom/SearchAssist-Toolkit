...
import {
  ...
  Upload, X, FileText, Download, Loader2, AlertCircle, Trash2, ChevronUp,
} from "lucide-react";
            ...
            <div key={gs.version} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              <div className="flex items-center gap-3 px-5 py-4">
                ...
                <div className="flex items-center gap-4 text-sm text-gray-500 shrink-0">
                  ...
                  <a
                    href={goldenSetsApi.exportUrl(appId!, gs.version)}
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="bg-gray-50 text-left">
                            <th className="px-4 py-2.5 font-medium text-gray-500 text-xs">Question</th>
                            <th className="px-4 py-2.5 font-medium text-gray-500 text-xs">Expected Behavior</th>
                            <th className="px-4 py-2.5 font-medium text-gray-500 text-xs">Case</th>
                            <th className="px-4 py-2.5 font-medium text-gray-500 text-xs">Type</th>
                            <th className="px-4 py-2.5 font-medium text-gray-500 text-xs">Difficulty</th>
                            <th className="px-4 py-2.5 font-medium text-gray-500 text-xs">Status</th>
                            <th className="px-4 py-2.5 font-medium text-gray-500 text-xs">Decision</th>
                          </tr>
                        </thead>
      ...
      >
        <td className="px-4 py-3 max-w-[260px]">
          <div className="flex items-center gap-1.5">
            ...
            <p className="truncate text-gray-700 text-sm">{tc.question}</p>
          </div>
        </td>
        <td className="px-4 py-3">
          <span className={cn(
            "text-xs px-2 py-0.5 rounded-full font-medium",
            tc.expected_behavior === "ANSWER" ? "bg-blue-50 text-blue-700"
            : tc.expected_behavior === "REFUSE" ? "bg-red-50 text-red-600"
        ...
        <td className="px-4 py-3">
          {tc.question_type
            ? <span className="text-xs px-2 py-0.5 bg-violet-50 text-violet-700 rounded-full">{tc.question_type}</span>
            : <span className="text-gray-300 text-xs">—</span>}
        </td>
        <td className="px-4 py-3 text-gray-500">
          {tc.difficulty != null ? <DifficultyBar level={tc.difficulty} /> : <span className="text-gray-300 text-xs">—</span>}
        </td>
        <td className="px-4 py-3">
          ...
          <td colSpan={7} className="px-5 py-4 bg-gray-50 border-t border-gray-100">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              ...
              <div className="space-y-3">
                {tc.primary_concern && (
                  <div>
                    <p className="font-medium text-gray-600 mb-1">Primary Concern</p>
                    <p className="text-gray-500">{tc.primary_concern}</p>
                  </div>
                )}
                {tc.rationale && (
                  <div>
                    <p className="font-medium text-gray-600 mb-1">Rationale</p>
                    <p className="text-gray-500 whitespace-pre-wrap leading-relaxed">{tc.rationale}</p>
                  </div>
                )}
                {!tc.primary_concern && !tc.rationale && (
                  <p className="italic text-gray-400">No agent rationale available</p>
                )}
                {tc.custom_fields && Object.keys(tc.custom_fields).length > 0 && (
                  <div>
                    <p className="font-medium text-gray-600 mb-1">Extra Columns</p>
                    <div className="space-y-1">
                      {Object.entries(tc.custom_fields).map(([k, v]) => (
